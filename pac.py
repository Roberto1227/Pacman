...
class Pac(pygame.sprite.Sprite):
	def __init__(self, row, col):
		super().__init__()
		self.walking_sound = pygame.mixer.Sound('assets/sounds/pacman-waka-waka.mp3')
	...
	def animate(self, pressed_key, walls_collide_list):
		...
		is_moving = self.status != 'idle'
		if is_moving and not self.sound_playing:
			self.walking_sound.play(loops=-1)
			self.sound_playing = True
		elif not is_moving and self.sound_playing:
